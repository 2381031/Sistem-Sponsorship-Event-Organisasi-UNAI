...
export default function DocumentGallery({ docs }: { docs: EventDoc[] }) {
  return <details className="rounded-xl border border-gray-100 bg-white p-3 text-xs">
    <summary className="cursor-pointer font-bold text-blue-800">Dokumentasi Kegiatan ({docs.length})</summary>
    <div className="mt-3 space-y-3">
      {docs.length === 0 && <p className="text-gray-500">Organisasi belum mengunggah dokumentasi kegiatan.</p>}
      {docs.map((doc, index) => {
        ...
        return <div key={doc.id_dokumentasi} className="space-y-2 rounded-lg bg-slate-50 p-3">
          ...
          {!url ? <p>Berkas lama perlu diunggah ulang oleh organisasi.</p> : <>
            ...
            <a href={url} target="_blank" rel="noreferrer" className="inline-block font-bold text-blue-700 underline">Buka {type === 'PDF' ? 'Dokumentasi PDF' : 'Berkas'}</a>
          </>}
        </div>;
      })}
    </div>
